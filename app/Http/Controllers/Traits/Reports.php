...
trait Reports
{
    ...
    public function filter()
    {
        if (session()->has('company_id')) {
            ...
            $company_id = $driver ? $driver->company_id : 0;
            if ($company_id) {
                session()->put('company_id', $company_id);
            }
        } else {
        ...
        if (session()->has('tvde_month_id')) {
            ...
            $tvde_month = TvdeMonth::orderBy('number', 'desc')
                ->when($company_id, function ($query) use ($company_id) {
                    $query->whereHas('weeks', function ($week) use ($company_id) {
        ...
        if (session()->has('tvde_week_id')) {
            ...
            $tvde_week = TvdeWeek::orderBy('number', 'desc')
                ->when($company_id, function ($query) use ($company_id) {
                    $query->whereHas('tvdeActivities', function ($tvdeActivity) use ($company_id) {
                        $tvdeActivity->where('company_id', $company_id);
                    });
                })
                ->when($tvde_month_id, function ($query) use ($tvde_month_id) {
                    $query->where('tvde_month_id', $tvde_month_id);
                })
                ->first();
            if ($tvde_week) {
        ...
        $tvde_years = TvdeYear::orderBy('name')
            ->when($company_id, function ($query) use ($company_id) {
                $query->whereHas('months', function ($month) use ($company_id) {
        ...
        $tvde_week = $tvde_week_id ? TvdeWeek::find($tvde_week_id) : null;

        $drivers = $company_id
            ? Driver::where('company_id', $company_id)->where('state_id', 1)->orderBy('name')->get()->load('team')
            : collect();
